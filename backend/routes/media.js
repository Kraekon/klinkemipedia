...
// GET /api/media/analytics - Get comprehensive analytics (must be before /:filename routes)
router.get('/analytics', protect, async (req, res) => {
  try {
    ...
    // Sort by usage and size
    const mostUsed = [...mediaList]
      .sort((a, b) => b.usageCount - a.usageCount)
      .slice(0, 10);

    const largest = [...mediaList]
      .sort((a, b) => b.size - a.size)
      .slice(0, 10);

    const unused = mediaList.filter(m => m.usageCount === 0);
    const used = mediaList.filter(m => m.usageCount > 0);
...
// DELETE /api/media/bulk - Bulk delete images (must be before /:filename routes)
router.delete('/bulk', protect, async (req, res) => {
  try {
    ...
    for (const imageId of imageIds) {
      try {
        // imageId could be filename or _id
        let filename = imageId;
        
        // Try to find in database first
        const media = await Media.findById(imageId).catch(() => null);
        if (media) {
        ...
        try {
          ...
          results.skipped.push({ imageId, reason: 'File not found' });
          continue;
        }
        ...
        if (usage.usageCount > 0) {
          results.skipped.push({ 
            imageId, 
            filename,
            reason: `Used in ${usage.usageCount} article(s)`,
            articles: usage.articles 
          });
          continue;
        }
        ...
        results.deleted.push({ imageId, filename });
      } catch (error) {
        results.failed.push({ 
          imageId, 
          error: error.message 
    ...
    res.json({
      success: true,
      message: `Bulk delete completed. Deleted: ${results.deleted.length}, Skipped: ${results.skipped.length}, Failed: ${results.failed.length}`,
      results
    });
  } catch (error) {
